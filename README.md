...
[![Deploy to Heroku](https://www.herokucdn.com/deploy/button.svg)](https://heroku.com/deploy?template=https://github.com/authorizerdev/authorizer-heroku/tree/with-postgres-redis)
...
| `REDIS_URL` | Redis connection string, used for sessions and shared rate limits | *(auto-configured by the heroku-redis add-on)* |
| `AUTHORIZER_URL` | This deployment's own public base URL **(required)**. Not the same as allowed origins — this is where Authorizer itself lives. The server exits at boot without it | `https://your-app.herokuapp.com` |
...
`https://your-app.herokuapp.com/app,https://yoursite.com/callback` — to get the exact matching
